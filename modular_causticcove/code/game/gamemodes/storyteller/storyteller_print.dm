/datum/storyteller/proc/storyteller_panel()
	...
	data += "<br>Major: [round(points[EVENT_LEVEL_CATASTROPHIC], 0.1)] / [POOL_THRESHOLD_CATASTROPHIC]   <a href='?src=\ref[src];modify_points=[EVENT_LEVEL_CATASTROPHIC]'>\[ADD\]</a>"
...
/* 	data += "<hr><b>Current antags:</b><div style=\"border:1px solid black;\"><ul>"
	...
	//Severity shit.
	var/global/list/severity_to_string = list("[EVENT_LEVEL_MUNDANE]" = "Mundane", "[EVENT_LEVEL_MINOR]" = "Minor", "[EVENT_LEVEL_MODERATE]" = "Moderate", "[EVENT_LEVEL_CATASTROPHIC]" = "Catastrophic","[EVENT_LEVEL_ROLESET]" = "Roleset")

	//This complex block will print out all the events with various info
	var/severity = EVENT_LEVEL_MUNDANE
	for(var/list/L in list(event_pool_mundane, event_pool_moderate, event_pool_catastrophic, event_pool_roleset))
...
/* 	if(href_list["force_spawn"])
	...
	if(href_list["edit_nobles"])
		nobles = input("Enter new head crew count.","Debug",nobles) as num

	if(href_list["edit_players"])
		players = input("Enter new total crew count.","Debug",players) as num

	if(href_list["edit_garrison"])
		garrison = input("Enter new security crew count.","Debug",garrison) as num

	if(href_list["edit_churchies"])
		churchies = input("Enter new medical crew count.","Debug",churchies) as num

	if(href_list["edit_yeomen"])
		yeomen = input("Enter new engineering crew count.","Debug",yeomen) as num

	if(href_list["edit_courtiers"])
		courtiers = input("Enter new science crew count.","Debug",courtiers) as num
...
/* 	if(href_list["call_shuttle"])
	...
	if(href_list["modify_points"])
		...
		var/add_points = input("Pool [pooltype] currently has [round(points[pooltype], 0.01)]. How many do you wish to add? Enter a negative value to subtract points","Altering Points",yeomen) as num
