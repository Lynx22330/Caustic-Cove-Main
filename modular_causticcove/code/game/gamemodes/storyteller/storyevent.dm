...
/datum/storyevent
	...
	var/req_churchies = -1
	var/req_courtiers = -1
	var/req_adventurers = -1

	var/max_players_diff_lower = 10	//Maximum difference between above values and real players distribution. If difference is greater, weight will be 0
	var/max_players_diff_higher = 10

	var/ocurrences = 0 //How many times this round, this storyevent has happened.
	var/ocurrences_max = -1 //How many times per round this can occur.
	...
	//EVENT_LEVEL_MAJOR
	//EVENT_LEVEL_ROLESET
	//EVENT_LEVEL_ECONOMY  (not implemented, yet)
...
/datum/storyevent/proc/can_trigger(var/severity, var/mob/report)
	...
	if (event_type)
		...
		var/datum/events/E = new event_type(src, severity)
		if (!E.can_trigger())
